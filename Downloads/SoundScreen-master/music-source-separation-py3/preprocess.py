...
# Batch considered
def get_random_wav(filenames, n_wavs, v_wavs, sec, sr=ModelConfig.SR):
    ...
    n = np.array(list(map(lambda f: _sample_range(_pad_wav(librosa.load(f, sr=sr, mono=True)[0], sr, sec), sr, sec), n_wavs)))
    v = np.array(list(map(lambda f: _sample_range(_pad_wav(librosa.load(f, sr=sr, mono=True)[0], sr, sec), sr, sec), v_wavs)))

    noise = np.array([n[0]])
    voice = np.array([v[0]])

    n_v = np.array( [ np.array( [ n[0] , v[0] ] ) ] )

    mixed = np.array(list(map(lambda f: librosa.to_mono(f), n_v)))

    return mixed, noise, voice

# Batch considered
def get_random_wav_eval(filenames, sec, sr=ModelConfig.SR):
    # load wav -> pad if necessary to fit sr*sec -> get random samples with len = sr*sec -> map = do this for all in filenames -> put in np.array
