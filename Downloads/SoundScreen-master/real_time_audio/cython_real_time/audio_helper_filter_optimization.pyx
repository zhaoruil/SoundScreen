...
cdef:
    ...
    int32_t[:, ::view.contiguous] recordedSamples
    int32_t[::view.contiguous]    playbackSamples
...
cdef int recordCallback(
    ...
    while i < BUFFER_SIZE:
        recordedSamples[i, 0] = rptr[0]  # left
        rptr += 1
...
cdef int playCallback(
    ...
    while i < BUFFER_SIZE:
        wptr[0] = playbackSamples[i]  # left
        wptr += 1
        
        wptr[0] = playbackSamples[i + 1]  # right
        wptr += 1
        i    += 2
...
cdef filter_audio():
    ...
    # moving average
    playbackSamples = (running_mean(tmp, windowSize)).astype(DTYPE)
...
cpdef int main():
    ...
    cdef np.ndarray[sample_t, ndim=2] npRecordArr   = np.zeros([FRAMES_PER_SAMPLE, NUM_CHANNELS], dtype=DTYPE, order='C')
    cdef np.ndarray[sample_t]         npPlaybackArr = np.zeros(BUFFER_SIZE, dtype=DTYPE, order='C')
    
    recordedSamples = npRecordArr
    playbackSamples = npPlaybackArr
